  ...
  return (
    <article className="prose prose-neutral dark:prose-invert prose-pre:bg-transparent prose-pre:p-0 mx-auto max-w-3xl px-4 pb-20 sm:px-6">
      ...
      <p>
        ...
        <a target="_blank" rel="noopener noreferrer" href="https://aws.amazon.com/rds/">
          AWS RDS
        </a>
        .
      </p>
      <p>
        ...
        balancing serving our web app with database operations. If that's the case, the solution is to offload your
        database operations to a dedicated machine. This architecture also enables scaling to multiple web servers
        in the future, where we might have multiple web servers talking to a centralized database which securely
        hosts our user data. In this blog post, we'll walk through scaling up from our SQLite database to a
      ...
      <ol>
        <li>
          ...
          <a
            ...
            href="https://us-west-1.console.aws.amazon.com/rds/"
          >
            AWS RDS console
          </a>{" "}
        ...
        <li>
          ...
          <code>.rds.amazonaws.com</code>) and port number (usually 5432) from the console and store them somewhere
          safely. It's bad practice to store them in code, so we'll store them in environment variables in our EC2
          instance for now:
...
`}
      ...
      <p>
        Ideally, we want to continue using SQLite while developing our app locally but point to RDS in production. We
        can enable this in our <code>settings.py</code> file by replacing the default values for{" "}
...
`}
      ...
      <p>
        This code will read values from the environment variables we created earlier and use them to connect to our
        RDS machine in production. Next, on your server, run:
      </p>
      <SyntaxHighlighter language="sh" style={monokaiSublime}>
      ...
      <p>
        If everything worked, our EC2 machine should connect to RDS and we should get a PostgreSQL prompt allowing us
        to query the newly created database. But hang on, there's no tables or data yet!
      </p>

      {/* Migrate Tables and Data to RDS */}
      <ResponsiveHeading numbering="03">Migrate Tables and Data to RDS</ResponsiveHeading>
      <p>
        Now that everything's connected, the final step is initializing our database with our tables and data. If we
        want to start fresh with an empty tables, simply run:
      </p>
      <SyntaxHighlighter language="sh" style={monokaiSublime}>
...
`}
      ...
      <p>
        ...
        setup for high availability, but those are reserved for another blog post. Congratulations, and happy coding!
      </p>
    </article>
  );
}

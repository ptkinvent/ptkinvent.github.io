...
export default function AwsAlb() {
  return (
    <article className="prose prose-neutral dark:prose-invert prose-pre:bg-transparent prose-pre:p-0 mx-auto max-w-3xl px-4 pb-20 sm:px-6">
      <p>
        This blog post is part of a series about how to deploy a Django app to AWS. If you haven't deployed your app
        to EC2 and RDS yet, I recommend reading the{" "}
        <Link href="/blog/aws-ec2">
          <span className="font-bold">AWS EC2</span>
        </Link>{" "}
        and{" "}
        <Link href="/blog/aws-rds">
          <span className="font-bold">AWS RDS</span>
        </Link>{" "}
        posts first.
      </p>
      <p>
        So your website's getting so much traffic that you're ready to scale your web app to multiple servers? Great!
        In this post, we'll walk through how to prepare our single EC2 server for scalability, create a template
        from it to spin up more instances, and create an AWS Application Load Balancer (ALB) to distribute traffic
        amongst them. Let's get started!
      </p>
...
sudo /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-config-wizard`}
        ...
        <li>
          Say <b>yes</b> to all defaults except "Do you want to store the config in the SSM parameter store?" and "Do
          you want the CloudWatch agent to also retrieve X-ray traces?"
        </li>
        <li>
      ...
      <p>
        ...
        <code>/opt/aws/amazon-cloudwatch-agent/bin/config.json</code>. Note that our configuration is also saved to{" "}
        <a target="_blank" rel="noopener noreferrer" href="https://console.aws.amazon.com/systems-manager/" className="font-bold">
          AWS Systems Manager
        </a>{" "}
        &gt; Parameter Store for us. When we launch the CloudWatch Agent, we can choose to launch it with a local
        JSON file or by grabbing a config from the parameter store. Grabbing from the parameter store is considered
        best practice, but for the simplicity of this tutorial we're using the local file. If we ever did want to
        fetch a config from the parameter store, we would use <code>-c ssm:&lt;parameterName&gt;</code> below
        instead.
      </p>
      <ol start="7">
        <li>
      ...
      <p>
        Great! Our CloudWatch Agent is now streaming our web server's logs to CloudWatch. We can check the status of
        the CloudWatch Agent on any of our EC2 machines at any time using:{" "}
        <code>amazon-cloudwatch-agent-ctl -a status</code>. If we navigate to{" "}
        <a target="_blank" rel="noopener noreferrer" href="https://console.aws.amazon.com/cloudwatch" className="font-bold">
          AWS CloudWatch
        </a>{" "}
        &gt; Log groups we should be able to see our logs streaming in. We now have a scalable way to monitor our web
        servers' usage.
      </p>
      ...
      <p>
        Currently, our secrets (such as database password, API keys, etc.) might be stored as environment variables
        on our EC2 machine. This also won't scale, since creating, updating, or deleting these secrets will get
        cumbersome as we increase the number of servers. More importantly, this also isn't a secure way to store
        secrets since anyone with access to our server can access these secrets. Instead, we'll use{" "}
        <a target="_blank" rel="noopener noreferrer" href="https://aws.amazon.com/secrets-manager/" className="font-bold">
          AWS Secrets Manager
        </a>{" "}
      ...
      <ol>
        <li>
          Navigate to{" "}
          <a className="font-bold" target="_blank" rel="noopener noreferrer" href="https://console.aws.amazon.com/iam/">
            AWS IAM console
          </a>{" "}
      ...
      <p>Now, let's create a test secret in Secrets Manager so we can try reading it from Django on our EC2 machine:</p>
      <ol start="4">
        <li>
          Navigate to{" "}
          <a
            ...
            href="https://console.aws.amazon.com/secretsmanager/"
          >
            AWS Secrets console
          </a>{" "}
      ...
      <p>
        Now that our secret is created, let's access it from Django. Keep in mind that good practice mandates using
        different secrets for development vs production&mdash;e.g. different API keys or database passwords. Let's
        continue using environment variables for local development so we aren't reliant on an Internet connection.
        Modify <code>settings.py</code> to look like the following:
      </p>
...
...`}
      ...
      <p>
        Great! Now that our EC2 machine is ready for scaling, let's create a template so that we can quickly launch
        more EC2 instances without having to painstakingly set each one up manually. There are various strategies
        for creating a template, such as creating a new Amazon Machine Image (AMI), which is a snapshot of our EC2
        machine, or a launch template. For this tutorial, we'll use the simplest strategy: creating a launch
        template.
      </p>
      <p>
        Launch templates allow us to quickly launch new EC2 instances with one click containing our latest code and
        other configuration files.
      </p>
      <ol>
        <li>
          Navigate to the{" "}
          <a className="font-bold" target="_blank" rel="noopener noreferrer" href="https://console.aws.amazon.com/ec2/">
            AWS EC2 console
          </a>
          .
        </li>
        ...
        <li>
          Finally, in <b>Advanced details</b> &gt; <b>IAM instance profile</b>, select the role you created for your
          EC2 machine.
        </li>
        <li>
          In <b>Advanced details</b> &gt; <b>User data</b>, add the following script to install and configure our web
          app:
          <SyntaxHighlighter language="sh" style={monokaiSublime}>
            {`#!/bin/bash
cd /home/ubuntu
...
sudo /opt/aws/amazon-cloudwatch-agent/bin/amazon-cloudwatch-agent-ctl -a fetch-config -m ec2 -c file:/opt/aws/amazon-cloudwatch-agent/bin/config.json -s`}
      ...
      <p>
        Great! Now we can easily launch multiple new web servers by selecting our template &gt; Actions &gt; Launch
        instance from template. Feel free to modify this template as you go, for example by adding extra setup
        instructions for npm by selecting our template &gt; Actions &gt; Modify template (Create new version).
      </p>
      ...
      <p>
        ...
        could add a custom middleware to intercept this request and put less load on our server, but let's just add
        a regular view instead for simplicity at the endpoint <code>/status</code>:
      </p>
      <ol>
        <li>
  ...
  path('status/', views.status_view, name='status'),
      ...
      <p>
        ...
        <code>/status</code> every 30 seconds. If we select the newly created target group, we should see a count of
        healthy and unhealthy web servers. By default, a server is marked as unhealthy if it fails to respond to 5
        pings in a row, and is marked healthy again when it responds to 2 pings in a row (all of this can be
      ...
      <p>
        Next, let's modify our security groups. Note that we want no longer want our EC2 instances exposed to public
        web traffic, only our load balancer. First, let's create a new security group for our ALB:
      </p>
      <ol start="10">
        <li>
          Navigate to the{" "}
      ...
      <ol start="13">
        ...
        <li>Add a new inbound rule that allows HTTP traffic from the security group of our ALB we just created.</li>
      </ol>
      <p>
        Finally, we're ready to create the AWS Application Load Balancer (ALB) which will distribute traffic to the
        web servers in our new target group:
      </p>
      <ol start="17">
        <li>
          Navigate to the{" "}
      ...
      <p>
        ...
        <code>dualstack.alb-&lt;random-id&gt;.us-west-1.elb.amazonaws.com</code>). Now that everything's hooked up,
        the final step is to point our domain to our ALB instead of to our single EC2 instance.
      </p>
      <ol start="24">
        <li>
          Select the ALB we just created and navigate to the <b>Details</b> tab. Copy the <b>DNS name</b> of the ALB.
        </li>
        <li>
      ...
      <p>
        Well done! We've now enabled multiple web servers and created a load balancer to distribute traffic amongst
        them.
      </p>
      ...
      <p>
        ...
        servers using our launch template to maintain a certain number of healthy servers at all times. It can also
        be configured to scale up and down servers based on the average usage (memory, CPU, or networking traffic)
        across our servers to keep up with traffic. Congratulations on completing this series of blog posts!
      </p>
    </article>
  );
}
